...
""",
    ...
    parser.add_argument("--input-glob", default="data/parsed/*.json", help="输入文件的 glob 模式")
    parser.add_argument("--force-types", "-t", help="强制指定类型（逗号分隔），跳过路由阶段")
    parser.add_argument("--min-score", type=float, default=0.4, help="最低分数阈值")
    parser.add_argument("--skip-validation", action="store_true", help="跳过 Schema 校验")
