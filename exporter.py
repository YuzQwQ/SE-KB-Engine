...
def _select_types(auto: bool, targets: list, text: str) -> list:
    ...
    signals = {
        ...
        "concepts": sum(
            text.count(k) for k in ["概念", "定义", "是指", "表示", "definition", "concept"]
        ),
    ...
    for f in files:
        ...
        for t in types:
            ...
            if artifact:
                try:
                    if t == "dfd":
                        ...
                        cc = len(
                            ((artifact.get("generation_knowledge") or {}).get("concepts") or [])
                        )
            ...
            if not ok or final_score < min_score:
                ...
                out = writer.write(
                    domain, title, parsed, text, t, None, trace, meta, metrics, errors
                )
                saved.append(out)
...
def main():
    ...
    p.add_argument("--input-glob", default="data/parsed/*.json", help="输入文件的 glob 模式")
    p.add_argument("--target", default="dfd,concepts,rules", help="目标类型，逗号分隔")
    p.add_argument("--auto", action="store_true", help="自动选择类型")
    p.add_argument("--min-score", type=float, default=0.5, help="最低分数阈值")
    args = p.parse_args()
