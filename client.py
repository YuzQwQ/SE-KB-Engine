...
class MCPClient:
    ...
    async def process_query(self, query: str) -> str:
        ...
        first_response = self.client.chat.completions.create(
            model=self.model, messages=messages, tools=available_tools, tool_choice="auto"
        )
        try:
        ...
        if first_message.tool_calls:
            for tool_call in first_message.tool_calls:
                ...
                tool_content = (
                    getattr(content_item, "text", None) if content_item is not None else None
                )
                if not tool_content:
                    tool_content = (
                        "服务暂时不可用喵~" if content_item is None else str(content_item)
                    )
    ...
    async def chat_loop(self):
        ...
        print(
            "您好，我是专业的网页爬取与分析助手，可以帮您进行各种网页内容的抓取、分析和结构化存储。"
        )
