...
def run_pipeline(parsed_data: Dict[str, Any], force_types: List[str] = None) -> PipelineResult:
    """
    便捷函数：运行两阶段抽取流水线
    ...
    Returns:
        PipelineResult
    """
    pipeline = ExtractionPipeline(skip_routing=bool(force_types), force_types=force_types)
    return pipeline.run(parsed_data)


def run_pipeline_from_file(file_path: str, force_types: List[str] = None) -> PipelineResult:
    """
    便捷函数：从文件运行流水线
