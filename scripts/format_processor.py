...
class FormatProcessor:
        ...
        return {
            "dfd": {
                ...
                "extraction_config": {
                    ...
                    "case_keywords": {"error": ["错误", "问题"], "best_practice": ["正确", "建议"]},
                },
            }
        }
    ...
    def extract_knowledge(self, content: str, url: str = "", title: str = "") -> Dict:
        ...
        # 从配置文件中获取知识库类别
        knowledge_categories = self.template.get("json_structure", {}).get(
            "knowledge_categories", []
        )
        ...
        for category in knowledge_categories:
            ...
            if category_key:
                extracted_data[category_key] = self._extract_by_category(
                    content, category_key, extraction_config, url, title
                )
    ...
    def _extract_by_category(
        ...
        if category_key == "dfd_concepts":
            ...
            extracted_items = self._extract_patterns_from_config(
                content, extraction_config, url, title
            )
    ...
    def _extract_patterns_from_config(
        self, content: str, extraction_config: Dict, url: str = "", title: str = ""
    ) -> List[Dict]:
        """从配置文件提取DFD模式"""
        patterns = []
    ...
    def _extract_nlp_mappings_from_config(
        self, content: str, extraction_config: Dict
    ) -> List[Dict]:
        """从配置文件提取DFD NLP映射"""
        mappings = []
    ...
    def generate_json_structure(self, extracted_data: Dict, url: str, title: str) -> Dict:
        ...
        # 构建JSON结构
        stats: Dict[str, int] = {}
        json_obj: Dict[str, Any] = {"metadata": metadata, "statistics": stats}
    ...
    def generate_markdown(self, extracted_data: Dict, metadata: Dict) -> str:
        ...
        for section in template["sections"]:
                ...
                if isinstance(section["content"], str):
                    md_lines.append(
                        self._format_template(
                            section["content"], {**metadata, "content_analysis": content_analysis}
                        )
                    )
                    ...
                    for line in section["content"]:
                        ...
                        categories = self.template["json_structure"]["knowledge_categories"]
                        stats = {
                            f"{category['key'].split('_')[-1]}_count": len(
                                extracted_data.get(category["key"], [])
                            )
                            for category in categories
                        }
            ...
            if "item_template" in section:
                section_key = (
                    section["title"].split("(")[-1].split(")")[0] if "(" in section["title"] else ""
                )
    ...
    def save_knowledge_base(
        self, data: Dict, base_filename: str = None, output_dir: str | Path = KNOWLEDGE_BASE_DIR
    ) -> Dict:
        """根据格式配置保存知识库数据到独立文件"""
        try:
            ...
            summary_data = {
                ...
                "knowledge_base_structure": {
                    cat["key"]: cat["description"] for cat in knowledge_categories
                },
            }
