...
class UniversalKnowledgeProcessor:
    ...
    def __init__(self, config_file: str = None):
        ...
        self.extraction_config = self.template.get("universal_knowledge_base", {}).get(
            "extraction_config", {}
        )

        # 加载系统提示词配置
        self.system_prompts_config = self._load_system_prompts_config()
        self.default_category = self.system_prompts_config.get(
            "default_prompt_type", "universal_knowledge"
        )
    ...
    def extract_knowledge(
        ...
        if not self._is_valid_text_content(content):
            logger.warning(
                f"Content validation failed for URL: {url}. Content appears to be binary or corrupted."
            )
            # 返回空的知识库结构
            return self._create_empty_knowledge_base(
                url, title, requirement_type, target_conversion_type
            )
    ...
    def _create_empty_knowledge_base(
        self,
        url: str = "",
        title: str = "",
        requirement_type: str = "",
        ...
        return {
            ...
            "validation_knowledge": {"criteria": [], "checklist": [], "error_patterns": []},
            "examples": {
                "input_examples": [],
                "output_examples": [],
    ...
    def _extract_concepts(self, content: str) -> List[Dict]:
        ...
        for sentence in sentences:
            ...
            if self._is_json_like_content(sentence):
                continue
            # 保留包含中文或有意义英文的句子
            if any("\u4e00" <= c <= "\u9fff" for c in sentence) or (
                len(sentence) > 20 and any(c.isalpha() for c in sentence)
            ):
    ...
    def _extract_concept_name_smart(self, sentence: str, indicators: List[str]) -> str:
        ...
        sentence_cleaned = re.sub(
            r"^(?:第\s*\d+[章节条]\s*|[一二三四五六七八九十百千]+[、.、]\s*|[一二三四五六七八九十百千]+\s+|\d+[、.、]\s*|\d+\s+)",
            "",
            sentence,
        )
    ...
    def _clean_content_for_extraction(self, content: str) -> str:
        ...
        # 移除常见的JSON字段名
        cleaned = re.sub(
            r"\b(position|title|link|redirect_link|displayed_link|favicon|date|snippet|snippet_highlighted_words|source|search_metadata|search_parameters|organic_results|pagination)\b:?",
            " ",
            cleaned,
        )
    ...
    def save_knowledge_base(
        self, knowledge_base: Dict, output_dir: str = KNOWLEDGE_BASE_DIR
    ) -> str:
    ...
    def convert_from_old_format(
        self, old_data: Dict, requirement_type: str = "", target_conversion_type: str = ""
    ) -> Dict:
        """从旧格式转换为新的通用知识库格式"""
        ...
        # 生成新的知识库ID
        knowledge_id = (
            f"converted_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{str(uuid.uuid4())[:8]}"
        )
