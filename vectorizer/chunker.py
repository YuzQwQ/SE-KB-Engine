...
class KnowledgeChunker:
    ...
    def _chunk_examples(self, data: Dict, file_path: Path) -> List[Chunk]:
        ...
        # 1. 案例概述块
        overview_text = self._build_case_overview(case_name, description, requirements, dfd_elements)
        chunks.append(Chunk(
    ...
    def _chunk_rules(self, data: Dict, file_path: Path) -> List[Chunk]:
        ...
        rules_list = (data.get("rules", []) or 
                      data.get("validation_rules", []) or
                      data.get("modeling_rules", []))
    ...
    def _build_case_overview(self, case_name: str, description: str, 
                             requirements: List[str], dfd_elements: Dict) -> str:
        """构建案例概述文本"""
        text_parts = [
            f"DFD案例: {case_name}",
            f"描述: {description}",
    ...
    def _build_process_text(self, proc_id: str, proc_name: str,
    ...
    def _calculate_complexity(self, dfd_elements: Dict) -> str:
        """计算案例复杂度"""
        total = (len(dfd_elements.get("external_entities", [])) +
