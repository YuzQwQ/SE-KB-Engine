...
class KnowledgeIndexer:
    ...
    def __init__(self, config: VectorConfig = None, log_callback: Callable[[str], None] = None):
    ...
    def index_file(self, file_path: Path) -> int:
        ...
        # 添加到向量库
        success = self.store.add_documents(collection_name, documents, metadatas, ids)
    ...
    def index_directory(self, dir_path: Path, recursive: bool = True) -> IndexStats:
        """
        索引目录
        ...
        json_files = [
            f
            for f in json_files
            if f.name not in exclude_names and not any(d in f.parts for d in exclude_dirs)
        ]
            ...
            try:
                ...
                if chunk_count > 0:
                    ...
                    stats.by_collection[collection_name] = (
                        stats.by_collection.get(collection_name, 0) + chunk_count
                    )
