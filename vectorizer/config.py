...
class VectorConfig:
    ...
    embedding_api_base: str = field(
        default_factory=lambda: os.getenv("EMBEDDING_BASE_URL", "https://api.siliconflow.cn/v1")
    )
    embedding_api_key: str = field(default_factory=lambda: os.getenv("EMBEDDING_API_KEY", ""))
    embedding_model_id: str = field(
        default_factory=lambda: os.getenv("EMBEDDING_MODEL_ID", "Qwen/Qwen3-Embedding-8B")
    )
    embedding_dimension: int = 4096

    # Collection 配置
    collections: Dict[str, Dict] = field(
