    ...
    def search(self, query: str, top_k: int = 5) -> PlannerResult:
        ...
        if len(sub_queries) == 1:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                ...
                futures = {
                    executor.submit(self._execute_single_search, sq, 3): sq for sq in sub_queries
                }
        ...
        return PlannerResult(original_query=query, sub_queries=sub_queries, merged_results=merged)

    def _merge_and_rank(
        self, results: List[RetrievalResult], final_top_k: int
    ) -> List[Dict[str, Any]]:
        """
        合并结果并使用加权 RRF 排序
        """
