...
class KnowledgeRetriever:
    ...
    def retrieve(
        ...
        if collections is not None:
            ...
            target_collections = self.INTENT_TO_COLLECTIONS.get(detected_intent) or list(
                self.store.collections.keys()
            )
        ...
        print(
            f"[Retrieval] Query: {query}, Intent: {detected_intent}, Collections: {target_collections}"
        )
            ...
            try:
                ...
                if results.get("documents") and results["documents"][0]:
                    for i, doc in enumerate(results["documents"][0]):
                        ...
                        print(
                            f"[Retrieval] Collection: {collection_name}, Distance: {distance:.4f}, Score: {score:.4f}, Min: {min_score}"
                        )
        ...
        return RetrievalResponse(
            query=query, intent=detected_intent, results=top_results, total_found=len(all_results)
        )
    ...
    def retrieve_examples(
        self, query: str, top_k: int = 5, complexity: str = None
    ) -> RetrievalResponse:
        """
        检索案例
        ...
        return self.retrieve(
            query, top_k, intent=QueryIntent.EXAMPLE, metadata_filter=metadata_filter
        )

    def retrieve_rules(self, query: str, top_k: int = 5, severity: str = None) -> RetrievalResponse:
        """
        检索规则
        ...
        return self.retrieve(query, top_k, intent=QueryIntent.RULE, metadata_filter=metadata_filter)

    def retrieve_templates(
        self, query: str, top_k: int = 5, dfd_level: int = None
    ) -> RetrievalResponse:
        """
        检索模板
        ...
        return self.retrieve(
            query, top_k, intent=QueryIntent.TEMPLATE, metadata_filter=metadata_filter
        )

    def retrieve_for_dfd_generation(
        self, requirements: str, top_k: int = 3
    ...
    def format_context(self, results: List[RetrievalResult], max_length: int = 4000) -> str:
        """
        将检索结果格式化为上下文文本
