...
class KnowledgeRefiner:
    ...
    def scan_artifacts(
        self, date_filter: str = None, time_filter: str = None
    ) -> List[ArtifactInfo]:
        """
        扫描 artifacts 目录
    ...
    def refine_type(
        self, type_id: str, artifacts: List[ArtifactInfo], stats: RefineStats
    ) -> List[Dict]:
        """
        精炼单个类型的知识
            ...
            try:
                ...
                if self.semantic_available and refined_knowledge:
                    is_sem_dup, sem_idx, sem_score = self.semantic_dedup.check_semantic_duplicate(
                        artifact.content, refined_knowledge
                    )
                ...
                structural_result = self.structural_dedup.check_duplicate(
                    artifact.content, refined_knowledge, type_id
                )

                if structural_result.is_duplicate:
                    self.log(
                        f"  → [结构层] 字段匹配重复 (匹配度: {structural_result.similarity_score:.2f})"
                    )
    ...
    def save_refined_knowledge(self, type_id: str, knowledge_list: List[Dict]):
        ...
        for knowledge in knowledge_list:
            ...
            # 移除内部字段
            output_knowledge = {
                k: v for k, v in knowledge.items() if not k.startswith("_file_path")
            }

            filepath.write_text(
                json.dumps(output_knowledge, ensure_ascii=False, indent=2), encoding="utf-8"
            )
            self.log(f"  保存: {filepath}")

    def run(
        self, date_filter: str = None, time_filter: str = None, dry_run: bool = False
    ) -> RefineStats:
        """
        执行完整的精炼流程
        ...
        for type_id, type_artifacts in groups.items():
            ...
            for i, artifact in enumerate(type_artifacts):
                result = self.structural_dedup.check_duplicate(
                    artifact.content, existing + [a.content for a in type_artifacts[:i]], type_id
                )
