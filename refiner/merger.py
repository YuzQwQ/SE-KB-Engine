...
class LLMMerger:
    ...
    def __init__(self, model_id: str = None):
        ...
        self.model_id = (
            model_id
            or os.getenv("MERGE_MODEL_ID")
            or os.getenv("KB_MODEL_ID")
...
请输出融合后的JSON："""
    ...
    def _call_llm(self, prompt: str, max_retries: int = 5) -> str:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "你是一个专业的知识融合助手，擅长将多个来源的知识整合为完整、准确的版本。",
                },
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_error = e
                print(
                    f"[LLMMerger] API 状态错误 (尝试 {attempt + 1}/{max_retries}): {e.response.status_code} - {e}"
                )
    ...
    def _add_provenance(
        self, merged: Dict, existing_source: str = None, new_source: str = None
    ) -> Dict:
        """添加来源追溯信息"""
        ...
        if existing_source and existing_source not in [s.get("url") for s in sources]:
            sources.append({"url": existing_source, "merged_at": now, "role": "existing"})

        if new_source and new_source not in [s.get("url") for s in sources]:
            sources.append({"url": new_source, "merged_at": now, "role": "new"})
        ...
        "elements": [
            {
                "id": "external_entity",
                "name": "外部实体",
                "definition": "位于系统边界之外，与系统进行数据交互的实体",
            },
