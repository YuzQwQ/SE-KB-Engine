...
class StructuralDeduplicator:
    ...
    def find_increments(self, new_item: Dict, existing_item: Dict, type_id: str) -> Dict:
        ...
        for path in primary_paths:
            ...
            existing_vals = set(str(v).lower() for v in self.extract_nested_values(existing_item, path) if v)
        ...
        for path in secondary_paths:
            ...
            if new_len > existing_len * 1.2:  # 新内容长度超过已有内容20%
                enhanced_fields.append({
                    "path": path,
                    "increase_ratio": new_len / max(existing_len, 1)
                })
    ...
    def check_duplicate(self, new_item: Dict, existing_items: List[Dict], type_id: str) -> DeduplicationResult:
        """
        检查新条目是否与已有条目重复
