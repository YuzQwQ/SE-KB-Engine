...
class WebpageCrawler:
    ...
    def __init__(self, data_dir: str = "data"):
        ...
        self.headers = {
            ...
            "Accept-Language": _rnd.choice(
                ["zh-CN,zh;q=0.9,en;q=0.8", "en-US,en;q=0.9,zh-CN;q=0.6"]
            )
            if use_random
    ...
    def _should_fallback(metrics: Dict[str, Any]) -> bool:
        ...
        if metrics.get("dynamic_markers_found") and metrics.get("cleaned_length", 0) < 2000:
            return True
        if (
            metrics.get("cleaned_length", 0) < 1200 or metrics.get("text_density", 0) < 0.18
        ) and metrics.get("main_selector_hits", 0) == 0:
            return True
        return False
    ...
    def parse_csdn_article(self, html_content: str, url: str) -> Dict[str, Any]:
        ...
        # 提取文章标题
        title = ""
        title_selectors = ["h1.title-article", ".article-title-box h1", "h1", ".title"]
        ...
        # 提取作者信息
        author = ""
        author_selectors = [".follow-nickName", ".username", ".author-name", ".user-info .name"]
        ...
        # 提取标签
        tags = []
        tag_selectors = [".tags-box .tag-link", ".article-tags a", ".tag"]
    ...
    def crawl_and_parse(
        ...
        url: str,
        save_data: bool = True,
        image_analysis_options: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        ...
        if not raw_data["success"]:
            ...
            human_sim = _os.getenv("PLAYWRIGHT_HUMAN_SIM", "1").strip() not in (
                "0",
                "false",
                "False",
            )
            ...
            rendered_html = fetch_rendered_html_sync(
                url, human_simulation=human_sim, proxy_server=proxy_server
            )
            if rendered_html:
                raw_data = {
                    "url": url,
        ...
        metrics_static = self._compute_metrics(
            raw_data["content"], structured_static.get("clean_text", ""), soup_static
        )
        ...
        if "csdn.net" in url:
            ...
            parsed_data = {**structured_static, "url": url, "parsed_at": datetime.now().isoformat()}
            parsed_data["word_count"] = len(parsed_data.get("clean_text", "") or "")
        ...
        if fallback_reason or self._should_fallback(metrics_static):
            ...
            if rendered_html:
                ...
                metrics_dyn = self._compute_metrics(
                    rendered_html, structured_dyn.get("clean_text", ""), soup_dyn
                )
                if "csdn.net" in url:
        ...
        if image_analysis_options is not None and parsed_data and parsed_data.get("images"):
            try:
                textual_images = analyze_images_fn(
                    parsed_data.get("images", []), url, image_analysis_options
                )
