...
class WebpageStorage:
    ...
    def _is_valid_image_url(self, img_url: str) -> bool:
        ...
        if any(
            ...
            for keyword in ["logo", "icon", "avatar", "ad", "ads", "spacer", "blank", "tracker"]
        ):
            return False
    ...
    async def _download_image(
        self, client: httpx.AsyncClient, img_url: str, save_path: Path
    ) -> Optional[Dict]:
        """下载单张图片"""
        try:
            # 检查文件大小
            ...
            if not original_filename or "." not in original_filename:
                original_filename = (
                    f"image_{hashlib.md5(img_url.encode()).hexdigest()[:8]}.{img_format}"
                )
    ...
    async def save_webpage(
        ...
        try:
                ...
                if valid_image_urls:
                    ...
                    for i, result in enumerate(download_results):
                        if isinstance(result, Exception):
                            failed_images.append({"url": valid_image_urls[i], "error": str(result)})
                        elif result:
                            downloaded_images.append(result)
                        else:
                            failed_images.append(
                                {"url": valid_image_urls[i], "error": "下载失败或图片无效"}
                            )
            ...
            logger.info(
                f"成功下载图片: {len(downloaded_images)}/{webpage_metadata['total_images']}"
            )
