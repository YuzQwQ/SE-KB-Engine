...
class HTMLCleaner:
    ...
    def __init__(self):
        ...
        self._rules_path = (
            Path(__file__).resolve().parent.parent / "config" / "html_cleaner_rules.json"
        )
    ...
    def _load_rules(self) -> Dict[str, Any]:
        ...
        default_rules = {
            ...
            "text_normalization": {"remove_emoji": False, "max_consecutive_newlines": 2},
            "markdown_options": {"table_style": "pipe", "infer_code_language": False},
            "domain_overrides": {},
        }
        try:
    ...
    def _locate_main_content(
        self, soup: "BeautifulSoup", base_url: str, rules: Dict[str, Any]
    ...
    def _ensure_continuity(
        self, soup: "BeautifulSoup", container: "BeautifulSoup", rules: Dict[str, Any]
    ) -> "BeautifulSoup":
    ...
    def _filter_images(
        self, images: List[Dict[str, str]], base_url: str, rules: Dict[str, Any]
    ) -> List[Dict[str, str]]:
        result = []
        exclude_keywords = [
            k.lower() for k in rules.get("image_filters", {}).get("filename_exclude_keywords", [])
        ]
        for img in images:
            src = img.get("src") or ""
            alt = img.get("alt", "")
    ...
    def clean_and_structure(self, html_content: str, base_url: str) -> Dict[str, Any]:
        ...
        for el in container.find_all("img"):
            raw_images.append(
                {"src": el.get("src") or el.get("data-src") or "", "alt": el.get("alt", "")}
            )
    ...
    def clean_html_with_structure(self, content: str) -> dict:
        ...
        try:
            ...
            return {"cleaned_content": cleaned_content, "title_structure": title_structure}

        except Exception as e:
            logger.warning(f"HTML结构化清理过程中出现错误: {str(e)}")
            # 如果结构化清理失败，至少返回基本清理结果
            return {"cleaned_content": self.clean_html_content(content), "title_structure": []}
    ...
    def _extract_title_structure(self, content: str) -> list:
        ...
        try:
            ...
            for tag in soup.find_all(["h1", "h2", "h3"]):
                ...
                if text:  # 只保留有内容的标题
                    title_structure.append({"level": level, "text": text, "tag": tag.name})
    ...
    def _extract_title_structure_regex(self, content: str) -> list:
        ...
        for tag, text in matches:
            ...
            if clean_text:
                title_structure.append({"level": level, "text": clean_text, "tag": tag.lower()})
    ...
    def get_cleaning_stats(self, original: str, cleaned: str) -> dict:
        ...
        reduction_ratio = (
            (original_length - cleaned_length) / original_length if original_length > 0 else 0
        )
