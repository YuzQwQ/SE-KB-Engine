...
class CrawlerFramework:
    ...
    def fetch_raw_data(
        self, engine: str, keyword: str, max_results: int = 10, **kwargs
    ) -> Dict[str, Any]:
        """
        从指定搜索引擎获取原始数据
        ...
        try:
            ...
            if engine_config.get("api_name") == "SerpAPI":
                raw_data = self._fetch_serpapi_data(
                    engine, keyword, max_results, engine_config, **kwargs
                )
            elif engine == "duckduckgo":
                raw_data = self._fetch_duckduckgo_data(
                    keyword, max_results, engine_config, **kwargs
                )
            else:
                return {"success": False, "error": f"引擎 {engine} 的API类型暂不支持"}
    ...
    def _fetch_serpapi_data(
        self, engine: str, keyword: str, max_results: int, config: Dict, **kwargs
    ) -> Dict:
        """使用SerpAPI获取数据"""
        ...
        params.update({"q": keyword, "api_key": self.serpapi_key, "num": max_results})
    ...
    def _fetch_duckduckgo_data(
        self, keyword: str, max_results: int, config: Dict, **kwargs
    ) -> Dict:
        """使用DuckDuckGo搜索获取数据"""
    ...
    def parse_results(
        self, raw_response: Dict[str, Any], engine: str = None, custom_rules: Dict = None
    ) -> Dict[str, Any]:
        """
        根据配置规则解析原始数据
        ...
        if not engine:
            return {"success": False, "error": "无法确定搜索引擎类型"}
        ...
        if not parsing_rules:
            return {"success": False, "error": f"引擎 {engine} 没有配置解析规则"}
        ...
        try:
            ...
            # 调试信息：打印原始数据结构
            logger.debug(
                f"[DEBUG] 原始数据键: {list(raw_data.keys()) if isinstance(raw_data, dict) else type(raw_data)}"
            )
            ...
            self._save_parsed_data(
                parsed_response, engine, raw_response.get("keyword"), raw_response.get("timestamp")
            )
            ...
            debug_info = {
                "error": str(e),
                "raw_data_structure": self._analyze_data_structure(raw_data)
            ...
            return {"success": False, "error": f"数据解析失败: {e}", "debug_info": debug_info}
    ...
    def _analyze_data_structure(self, data: Any, max_depth: int = 3) -> Dict:
        ...
        if isinstance(data, dict):
            ...
            return {
                ...
                "length": len(data),
                "sample_items": [
                    self._analyze_data_structure(item, max_depth - 1) for item in data[:2]
                ],
            }
        else:
            return {
    ...
    def search_and_parse(
        self, engine: str, keyword: str, max_results: int = 10, custom_rules: Dict = None, **kwargs
    ) -> Dict[str, Any]:
        """
        一站式搜索和解析
